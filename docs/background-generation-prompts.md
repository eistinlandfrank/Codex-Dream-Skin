...
| 你要做的事 | 应看的文件 | 说明 |
|---|---|---|
| 直接切换当前置顶主题 | macOS：`macos/presets/preset-arina-hashimoto/theme.json` + `background.jpg`；Windows：`windows/assets/theme.json` + `toki-reference.png` | macOS 播种「桥本有菜」，Windows 只内置 Toki；都不是 `docs/images/presets/*-light/dark` 截图 |
