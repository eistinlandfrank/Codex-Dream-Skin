...
Windows keeps the same generic local theme store and system-tray controls, but
the distribution bundles only the Toki bunny preset. For the first repo-based setup:
...
Toki is active immediately after launch. The same installer also deploys the
bundled Toki Bunny v2 animated pet; select it once through **Settings → Pets →
Refresh** on a new PC. **更换背景图** still imports your own UI-free wallpaper,
which can then be saved for one-click switching. The Arina Hashimoto preset
described above remains a macOS-only bundled preset.
