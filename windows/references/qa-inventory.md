...
- Home cards: all four actions are visible and keyboard-focusable on first paint. When Codex has not supplied native suggestions yet, the Toki fallback cards insert a prompt into the native composer without sending it; when native cards arrive, the fallback is removed.
...
- Tray lifecycle: pause/resume reflects the clicked state, Toki is the only bundled theme on first install, and complete restore terminates any separately launched tray before it can reapply the skin.
