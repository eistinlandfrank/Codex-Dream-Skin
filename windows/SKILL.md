...
description: Apply, launch, verify, repair, update, or restore the Toki decorative skin for the Windows Codex desktop app. Use when the user asks for the bundled Toki theme, needs the skin reapplied after a Codex update, or needs a safe rollback without modifying WindowsApps or app.asar.
---
...
1. Install Node.js 22 or newer, close Codex, then run `scripts/install-toki-dream-skin.ps1` once. The installer leaves the shared Codex configuration unchanged, seeds Toki as the only bundled Windows preset, validates and installs the bundled Toki Bunny v2 pet, copies the runtime to `%LOCALAPPDATA%\CodexDreamSkin\engine`, and creates Toki launch/restore/tray shortcuts that do not depend on the source checkout. On a new PC, explicitly select the pet once through Settings > Pets > Refresh.
2. Use the `Toki Codex（飞鸟马时）` shortcut, or run `%LOCALAPPDATA%\CodexDreamSkin\engine\scripts\start-dream-skin.ps1`. The shortcut asks before restarting an already-open Codex app; CLI callers must explicitly add `-RestartExisting`.
...
- `assets/toki-reference.png`: UI-free Toki wallpaper seeded as the default and as the only bundled saved preset.
- `assets/theme.json`: Toki metadata plus the shared adaptive theme contract for the seeded preset.
- `pets/toki-bunny/package/`: the only bundled Codex v2 desktop-pet runtime package; QA evidence and artwork notice live beside it.
- `scripts/install-toki-pet.ps1`: strict, repeat-safe validation and atomic deployment of the bundled pet without changing the selected pet.
