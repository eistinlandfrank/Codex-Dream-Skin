...
- 桌宠的运行任务数字不再随 Codex 原生临时通知的超时而消失：主窗口以低频通道广播侧栏中的真实运行任务，桌宠窗口仅在原生提示不可见时显示同等计数的备用数字框；原生提示恢复或任务结束后自动撤下。监听器只关注通知节点增删并避免重复 DOM 写入，以控制 CPU 占用。
...
- Toki 首页卡片在首帧即可点击，侧栏恢复 Codex 原生文件夹 SVG 与原版圆润无衬线字标；任务页沿用同一背景，移除高开销模糊、固定背景和持续动画。
